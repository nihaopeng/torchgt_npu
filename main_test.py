...
def main():
    ...
    # 参数设置 (后续可放入 args)
    METIS_K = 16        # 叉数
    METIS_DEPTH = 1    # 深度 (决定窗口大小: N / 2^Depth)
                       # 深度越大, Batch越小。
    DYN_THRESHOLD = 0.2 
    
    # 用于存储当前的训练分区列表 (List[List[int]])
    # 初始状态为空, 等待 rank 0 生成
    partitions_features = []  # 特征列表
    partitions_node_idlist = []       # ID列表
    root_tree_node = None

    if args.rank == 0:
        print(f"--- Initializing Partition Tree (K={METIS_K}, Depth={METIS_DEPTH}) ---")
        t0_partition = time.time()
        #    注意：这里传入 train_idx，只对训练集进行初始划分
        (partitions_features, 
         partitions_node_idlist, 
         root_tree_node) = partition_nodes_metis(
            feature=feature,
            edge_index=edge_index,
            max_depth=METIS_DEPTH,
            num_partitions=METIS_K,
            train_idx=train_idx
        )
        t_partition_cost = time.time() - t0_partition
        print(f"--- Partition Done. Cost: {t_partition_cost:.4f}s. Created {len(partitions_node_idlist)} partitions. ---")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch > 1 and epoch % 5 == 0 and args.rank == 0:
             print(f"--- Epoch {epoch}: Running Dynamic Window Adjustment ---")
             
             t0_adjust = time.time()

             (new_feats, new_ids) = dynamic_window_build(
                scores_partitions=last_epoch_scores,
                root_TreeNode=root_tree_node,
                partitions=partitions_node_idlist, # 传入当前的训练分区ID
                feature=feature,                   # 传入全局特征
             ...
             partitions_features = new_feats
             partitions_node_idlist = new_ids
             t_adjust_cost = time.time() - t0_adjust
             last_epoch_scores = []
             print(f"--- Adjustment Done. Cost: {t_adjust_cost:.4f}s. New partitions: {len(partitions_node_idlist)} ---")

        current_epoch_scores = []
        for partitions_id, node_ids in enumerate(partitions_node_idlist):
            if len(node_ids) == 0:
                continue
            
            # 准备顶点的索引 (List[int] -> Tensor)
            idx_i = torch.tensor(node_ids, dtype=torch.long)
            
            # 保持原逻辑
            #    数据打包 
            #    它负责提取子图结构、计算 Spatial Bias、Padding 等
            #    注意: 我们把这个分区当做一个 seq_len 不固定的 Batch
            
            # #t0_prepare = time.time()
            # packed_data = get_batch_reorder_blockize(
            #     args, feature, y, idx_i.to("cpu"), 
            #     sub_split_seq_lens, device, edge_index, N, 
            #     k=8, block_size=16, beta_coeffi=beta_coeffi_list[beta_idx]
            # )
            ...
            # 把所有层的分数加起来
            # Stack: [Layers, N, N] -> Sum: [N, N]
            batch_score = torch.stack(SCORES_COLLECTOR).sum(dim=0)
            current_epoch_scores.append(batch_score)
        ...
        last_epoch_scores = current_epoch_scores    #保存本轮分数
